...
- Exception: Jake's own reader tools (One List in Chapter Nineteen, The Pessimist in Chapter Twelve, and future suite members) may be named and offered in their chapters. Frame them as free tools where true. The governing rule (changed 18 July 2026, replacing the old only-one-pitch promise): Chapter Nineteen now makes a transparency promise instead - whenever the text points the reader at something of Jake's own, it must say so plainly, every time. Any mention of a Liddell product or service must carry that flag; the register to copy is the HighLevel reseller disclosure and the "it is mine, so weigh this entry accordingly" line in the tools directory.
