...
Banned outright: em dashes (—), en dashes (–), curly/smart quotes (" " ' '), the ellipsis character (…), arrows (→), and any other typographic substitute. Do not replace an em dash with "--" either: rewrite the sentence using a comma, colon, full stop, or parentheses, as the existing chapters do.
