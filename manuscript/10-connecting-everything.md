...
For each automation, consider having it log what it's doing so that a dashboard can agregate and display daily or weekly summaries.  This log will be invaluable over time.  You get to see what is normal, and in time you can set up guardrails that alert you if something falls outside "normal" parameters.
