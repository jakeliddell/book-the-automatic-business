...
> **Try this.** Get out the bottleneck list you made in Chapter One, the one with every task that stops and waits for you. Go down it and write a single letter beside each item. H if it truly needs a human. A if it is really just a rule. And a question mark beside anything that feels like the fuzzy middle. Do not try to solve anything yet. Just sort.  If, when reading these tasks, you now see they are a chain of tasks, break them up now, before applying the letter.You are doing triage on your own business for the first time, and the shape of what comes next is already there on the page. 
