...
A word of advice before the list. Choose the category first, then the tool. Decide what job needs doing, "I need a no-code connector," or "I need one place for client relationships," and only then pick a product to fill it. Pick one per job, learn it properly, and resist the urge to switch every time something shinier appears.  S The names below are current at the time of writing; check for the latest before you commit, because some of them will have changed by the time you read this.
