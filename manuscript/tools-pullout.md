...
You will also need a task / issue tracking tool, irrespective of whether that's already in your delivery tool.  There are always a hundred tasks open for a business that are not anything to do with Options include Asana (clean, widely used, easy for a team to adopt), Trello (the simplest: boards and cards, and sometimes that is all you need), monday.com (flexible and visual, at the cost of some clutter), and ClickUp (does everything, which is both the appeal and the risk).
