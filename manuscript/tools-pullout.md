...
As a previous reseller of ActiveCampaign, an implementor of Salesforce, someone who tried to use PipeDrive (it didn't work well), and done a handful of Hubspot migrations, my recommendation is HighLevel, which is why we currently resell it.  (For full transparency, we used to be an ActiveCampaign reseller, but moved away once we could no longer recomme
